...
Rpc.getTxInfo = function(txid, doNotParse, cb) {
  ...
  bitcoreRpc.getRawTransaction(txid, 1, function(err, txInfo) {
    ...
    //Included in BlockHeight

    return bitcoreRpc.getBlock(info.blockhash, function(err, binfo){
      // Not found?
      if (err && err.code === -5) return cb();
      if (err) return cb(self.errMsg(err));
      info.blockheight = binfo.result.height;
      info.switchheight = config.switch_height;
      return cb(null,info);
    });
  });
};
